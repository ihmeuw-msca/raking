...
def raking_loss(
    ...
    while (epsilon > tol) & (iter_eps < max_iter):
        ...
        loss_gradient = compute_loss(c - np.matmul(C, beta_n), loss, 'gradient')
        ...
        J = np.concatenate( \
            (np.concatenate( \
             ...
             np.concatenate( \
                 ...
                 np.matmul(C * conjugate_dist_hess, np.transpose(C)) + np.diag(conjugate_loss_hess / penalty)), axis=1) \
            ), axis=0)
...
def raking_dual_loss_scipy(
    ...
    def objective(lambda_k):
        ...
        return np.dot(lambda_1, s) - np.dot(lambda_2, c) + conjugate_dist + penalty * conjugate_loss
