...
from set_inequality_problems import set_time_trend
from raking_loss import raking_chi2_loss
...
for sex in sexes:
    for age in ages:
        ...
        (beta, lambda_k, iter_eps) = raking_chi2_loss(y, A, s, C, c, q, method='chi2', loss='logit', penalty=1.0)
        # Add results to data frame
        df_res = df_county.copy().sort_values(by=['year', 'race', 'mcnty', 'area'])
        df_res['raked_value'] = beta
        df_res['age'] = age
        df_res['sex'] = sex
        df_raked.append(df_res)

# Save in output file
df_raked = pd.concat(df_raked)
df_raked.to_csv('/ihme/homes/ducela/stash/raking/scripts_dev/obesity/raked_penalty.csv', index=False)
