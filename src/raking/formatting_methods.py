...
def format_data_1D(
    ...
    lower: str = None,
    upper: str = None
) -> tuple[np.ndarray, float, int, np.ndarray | None, np.ndarray | None, np.ndarray | None]:
    ...
    assert df_margins['value_agg_over_' + var_name].isna().sum() == 0, \
        'There are missing values in the value_agg_over' + var_name + ' column of the margins.'
...
def format_data_2D(
    ...
    lower: str = None,
    upper: str = None
) -> tuple[np.ndarray, np.ndarray, np.ndarray, int, int, np.ndarray | None, np.ndarray | None, np.ndarray | None]:
...
def format_data_3D(
    ...
    lower: str = None,
    upper: str = None
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, int, int, int, np.ndarray | None, np.ndarray | None, np.ndarray | None]:
    ...
    # Check the first margins data
    assert df_margins_1[var_names[1]].isna().sum() == 0, \
        'There are missing values in the ' + var_names[1] + ' column of the first margins.'
    assert df_margins_1[var_names[2]].isna().sum() == 0, \
        'There are missing values in the ' + var_names[2] + ' column of the first margins.'
    assert df_margins_1['value_agg_over_' + var_names[0]].isna().sum() == 0, \
    ...
    # Check the second margins data
    assert df_margins_2[var_names[0]].isna().sum() == 0, \
        'There are missing values in the ' + var_names[0] + ' column of the second margins.'
    ...
    assert df_margins_3[var_names[0]].isna().sum() == 0, \
        'There are missing values in the ' + var_names[0] + ' column of the third margins.'
