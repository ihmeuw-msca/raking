...
from compute_constraints import constraints_USHD
...
@profile
def raking_entropic(y, q, A, s, gamma0=1.0, max_iter=500):
    """
    Raking using the entropic distance beta log(beta/y) + y - beta.
    Input:
      y: 1D Numpy array, observed values y_n = y_i,j if n = j I + i
      q: 1D Numpy array, weights for the observations
      A: 2D Numpy array, linear constraints
      s: 1D Numpy array, partial sums
      gamma0: scalar, initial value for line search
      max_iter: Integer, number of iterations for Newton's root finding method
    Output:
      betas: list of 1D Numpy arrays, raked values
      lambdas: list of 1D Numpy arrays, dual
      iters_eps: list of integers, number of iterations until convergence
    """
    assert isinstance(y, np.ndarray), \
    ...
    while (epsilon > 1.0e-10) & (iter_eps < max_iter):
        Phi = np.matmul(A, y * (1 - np.exp(- q * \
            np.matmul(np.transpose(A), lambda_k))))
        D = np.diag(y * q * np.exp(- q * \
            np.matmul(np.transpose(A), lambda_k)))
...
def main():
    ...
    K = 254
    sigma = 0.1
    (beta, beta_i) = create_test_matrix(I, J, K)
    y = add_noise(beta, sigma)
    (A, s) = get_constrained_matrix_vector(beta_i, I, J, K)
    q = np.ones(len(y))
    (beta_chi2, lambda_chi2) = raking_chi2(y, q, A, s)
    beta_chi2_svd = beta_chi2[0]
    beta_chi2_cg1 = beta_chi2[1]
    beta_chi2_cg2 = beta_chi2[2]
    beta_chi2_cg3 = beta_chi2[3]
    beta_chi2_cg4 = beta_chi2[4]
    beta_chi2_minres1 = beta_chi2[5]
    beta_chi2_minres2 = beta_chi2[6]
    beta_chi2_minres3 = beta_chi2[7]
    beta_chi2_minres4 = beta_chi2[8]
    (beta_entropic, lambda_entropic, num_iters) = raking_entropic(y, q, A, s)
    beta_entropic_svd = beta_entropic[0]
    beta_entropic_cg1 = beta_entropic[1]
    beta_entropic_cg2 = beta_entropic[2]
    beta_entropic_cg3 = beta_entropic[3]
    beta_entropic_cg4 = beta_entropic[4]
    beta_entropic_minres1 = beta_entropic[5]
    beta_entropic_minres2 = beta_entropic[6]
    beta_entropic_minres3 = beta_entropic[7]
    beta_entropic_minres4 = beta_entropic[8]
    num_iters_svd = num_iters[0]
    num_iters_cg1 = num_iters[1]
    num_iters_cg2 = num_iters[2]
    num_iters_cg3 = num_iters[3]
    num_iters_cg4 = num_iters[4]
    num_iters_minres1 = num_iters[5]
    num_iters_minres2 = num_iters[6]
    num_iters_minres3 = num_iters[7]
    num_iters_minres4 = num_iters[8]
    ...
    print('\n')
    print('MAPE between beta chi2 - svg and cg1:', \
          np.mean(np.abs((beta_chi2_svd - beta_chi2_cg1) / beta_chi2_svd)))
    print('MAPE between beta chi2 svg and cg2:', \
          np.mean(np.abs((beta_chi2_svd - beta_chi2_cg2) / beta_chi2_svd)))
    print('MAPE between beta chi2 svg and cg3:', \
          np.mean(np.abs((beta_chi2_svd - beta_chi2_cg3) / beta_chi2_svd)))
    print('MAPE between beta chi2 svg and cg4:', \
          np.mean(np.abs((beta_chi2_svd - beta_chi2_cg4) / beta_chi2_svd)))
    print('MAPE between beta chi2 - svg and minres1:', \
          np.mean(np.abs((beta_chi2_svd - beta_chi2_minres1) / beta_chi2_svd)))
    print('MAPE between beta chi2 svg and minres2:', \
          np.mean(np.abs((beta_chi2_svd - beta_chi2_minres2) / beta_chi2_svd)))
    print('MAPE between beta chi2 svg and minres3:', \
          np.mean(np.abs((beta_chi2_svd - beta_chi2_minres3) / beta_chi2_svd)))
    print('MAPE between beta chi2 svg and minres4:', \
          np.mean(np.abs((beta_chi2_svd - beta_chi2_minres4) / beta_chi2_svd)))
    print('\n')
    print('MAPE between beta entropic - svg and cg1:', \
          np.mean(np.abs((beta_entropic_svd - beta_entropic_cg1) / beta_entropic_svd)))
    print('MAPE between beta entropic - svg and cg2:', \
          np.mean(np.abs((beta_entropic_svd - beta_entropic_cg2) / beta_entropic_svd)))
    print('MAPE between beta entropic - svg and cg3:', \
          np.mean(np.abs((beta_entropic_svd - beta_entropic_cg3) / beta_entropic_svd)))
    print('MAPE between beta entropic - svg and cg4:', \
          np.mean(np.abs((beta_entropic_svd - beta_entropic_cg4) / beta_entropic_svd)))
    print('MAPE between beta entropic - svg and minres1:', \
          np.mean(np.abs((beta_entropic_svd - beta_entropic_minres1) / beta_entropic_svd)))
    print('MAPE between beta entropic - svg and minres2:', \
          np.mean(np.abs((beta_entropic_svd - beta_entropic_minres2) / beta_entropic_svd)))
    print('MAPE between beta entropic - svg and minres3:', \
          np.mean(np.abs((beta_entropic_svd - beta_entropic_minres3) / beta_entropic_svd)))
    print('MAPE between beta entropic - svg and minres4:', \
          np.mean(np.abs((beta_entropic_svd - beta_entropic_minres4) / beta_entropic_svd)))
    print('\n')
    print('Difference between beta svd - chi2 and entropic:', \
          np.mean(np.abs((beta_chi2_svd - beta_entropic_svd) / beta_chi2_svd)))
