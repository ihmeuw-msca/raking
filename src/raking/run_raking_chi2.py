...
# Raking with inequality constraints without weights (chi2 distance)
males_raked_inequality = []
for year in years:
    # Select values for year
    males_loc = males_mean.loc[males_mean.year==year]
    df_01m = males_loc[['value', 'loc_id', 'mx mean', 't mean', 'worldpop', 'year', 'age_group', 'gbd_loc_value']]. \
        loc[males_loc.age_group=='0 to 1m']
    df_01y = males_loc[['value', 'loc_id', 'mx mean', 't mean', 'worldpop', 'year', 'age_group', 'gbd_loc_value']]. \
    ...
    males_raked_inequality.append(df_raked)
males_raked_inequality = pd.concat(males_raked_inequality)

males_raked_inequality['mx raked'] = males_raked_inequality['raked_value'] / males_raked_inequality['worldpop']
males_raked_inequality['qx raked'] = 1.0 - np.exp(- males_raked_inequality['t mean'] * males_raked_inequality['mx raked'])

# Save output
males_raked_inequality.to_csv('/ihme/homes/ducela/stash/raking/scripts_dev/males_raked_inequality.csv', index=False)
...
# Raking with inequality constraints without weights (chi2 distance)
females_raked_inequality = []
for year in years:
    # Select values for year
    females_loc = females_mean.loc[females_mean.year==year]
    df_01m = females_loc[['value', 'loc_id', 'mx mean', 't mean', 'worldpop', 'year', 'age_group', 'gbd_loc_value']]. \
        loc[females_loc.age_group=='0 to 1m']
    df_01y = females_loc[['value', 'loc_id', 'mx mean', 't mean', 'worldpop', 'year', 'age_group', 'gbd_loc_value']]. \
    ...
    df_01y['raked_value'] = beta2
    df_raked = pd.concat([df_01m, df_01y])
    females_raked_inequality.append(df_raked)
females_raked_inequality = pd.concat(females_raked_inequality)

females_raked_inequality['mx raked'] = females_raked_inequality['raked_value'] / females_raked_inequality['worldpop']
females_raked_inequality['qx raked'] = 1.0 - np.exp(- females_raked_inequality['t mean'] * females_raked_inequality['mx raked'])

# Save output
females_raked_inequality.to_csv('/ihme/homes/ducela/stash/raking/scripts_dev/females_raked_inequality.csv', index=False)
