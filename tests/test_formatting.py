...
def test_format_data_2D():
    ...
    df_margins_1 = pd.DataFrame({'var2': np.arange(0, J), 'value_agg_over_var1': s1})
    df_margins_2 = pd.DataFrame({'var1': np.arange(0, I), 'value_agg_over_var2': s2})
    # Get the formatted data
    (y, s1, s2, I, J, q, l, h) = format_data_2D(df_obs, df_margins_1, df_margins_2, ['var1', 'var2'])
    # Generate the constraints
...
def test_format_data_3D():
    ...
    df_obs = pd.DataFrame({'value': y, 'var1': var1, 'var2': var2, 'var3': var3})
